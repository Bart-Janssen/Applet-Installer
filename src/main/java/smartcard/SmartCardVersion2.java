...
import smartcard.channel.factory.scp.SCP02ChannelFactory;
...
public class SmartCardVersion2 extends SmartCard
{
    public SmartCardVersion2(CardTerminal reader, byte[] cardManager, Map<String, byte[]> keySet) throws Exception
    {
        super(reader, cardManager, keySet);
        super.supportedChannels.put(ChannelType.PLAIN, new ChannelFactoryPlain());
        super.supportedChannels.put(ChannelType.MAC, new SCP02ChannelFactory());
    ...
    public void installApplet(String applet) throws Exception
    {
        ...
        //GP Platform 2.3.1 chapter 11.5.2.3.1
    ...
    public void openSecureChannel(ChannelType channelType) throws Exception
    {
        ...
        int keyVersion = 0x00;
        CommandAPDU initUpdateCommand = new APDU(0x80,0x50, keyVersion, 0x00, hostChallenge, 0x00).create();
        ...
        System.arraycopy(cardChallenge, 0, derivationData, 0, 2);

        Map<String, byte[]> sessionKeys = new HashMap<>();
        sessionKeys.put("session-enc", this.calculateSessionKey(derivationData, super.keySet.get("ENC"),false));
        sessionKeys.put("session-mac", this.calculateSessionKey(derivationData, super.keySet.get("MAC"),true));
    ...
    //GlobalPlatform secure Channel Protocol '02'
    ...
    //GlobalPlatform E.4.1 DES Session Keys
    private byte[] calculateSessionKey(byte[] sequence, byte[] key, boolean mac) throws Exception
    {
        byte[] derivationData = new byte[16];
        System.arraycopy(sequence, 0, derivationData, 2, 2);
        byte[] SCP02_CONSTANT_MAC = mac ? new byte[]{0x01,0x01} : new byte[]{0x01,(byte)0x82};
        System.arraycopy(SCP02_CONSTANT_MAC, 0, derivationData, 0, 2);
        byte[] iv = new byte[8];
        return super.crypto.encryptTDES_CBC(derivationData, key, iv);
    }
}